...
class ApiTest(TestCase):
    ...
    def test_api_data_post(self):
        data = {"continuous_feature1": 1.23, "continuous_feature2": 1.88, "categorical_feature": 1}
    ...
    def test_api_data_post_invalid_data(self):
        data = {"continuous_feature1": 1.23, "continuous_feature2": 1.88, "categorical_feature": "invalid"}
        response = self.client.post('/api/data', data)
    ...
    def test_api_data_get_with_data(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), [
            {'id': 1, 'continuous_feature1': 1.23, 'continuous_feature2': 1.88, 'categorical_feature': 1}])
    ...
    def test_api_predictions_get(self):
        for i in range(10):
            data = {"continuous_feature1": random.uniform(0, 1000), "continuous_feature2": random.uniform(0, 1000),
                    "categorical_feature": random.randint(1, 3)}
            self.client.post('/api/data', data)

        response = self.client.get('/api/predictions?continuous_feature1=1.23&continuous_feature2=1.88')
        self.assertEqual(response.status_code, 200)
    ...
    def test_api_predictions_get_invalid_data(self):
        response = self.client.get('/api/predictions?continuous_feature1=1.23&continuous_feature2=invalid')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json().get('error'), 'Invalid data')
