    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseBadRequest(
                render(request, 'error_page.html', {'error_code': '400'}))
...
def predict(request):
    if request.method == 'POST':
        ...
        continuous_features = [
            [entry.continuous_feature1, entry.continuous_feature2] for entry in
            all_entries]
        categorical_feature = [entry.categorical_feature for entry in
                               all_entries]
...
def api_predictions(request):
    ...
    new_sample = scaler.transform(
        [[serializer.validated_data['continuous_feature1'],
          serializer.validated_data['continuous_feature2']]])
